            ...
            if (datastore.api_key === "keiyakudaicho") {
                ...
                for (let n = 0; n < ics.length; n++) {
                    ...
                    await ic.updateOne(
                        ...
                        {$set:{
                           'items.backup_sonnekigaku' :{'data_type' :'number','value' : message.items.sonnekigaku.value},
                           'items.backup_hkkjitenzan' :{'data_type' :'number','value' : message.items.hkkjitenzan.value},
                           'items.sonnekigaku' :{'data_type' :'number','value' : parseFloat(message.items.sonnekigaku.value)},
                           'items.hkkjitenzan' :{'data_type' :'number','value' : parseFloat(message.items.hkkjitenzan.value)}
                        }
                        }
                    );
                }
