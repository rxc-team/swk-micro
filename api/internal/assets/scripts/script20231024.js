        ...
        for (let j = 0; j < ds.length; j++) {
            ...
            if (datastore.api_key === "keiyakudaicho") {
                ...
                for (let n = 0; n < ics.length; n++) {
                    const message = ics[n];
                    if (message.items.sonnekigaku == null) {
                        await ic.updateOne(
                            {item_id:message.item_id},
                            {$set:{
                               'items.sonnekigaku' :{'data_type' :'number','value' : 0},
                            }
                            }
                        );
                    }
                    if (message.items.hkkjitenzan == null) {
                        await ic.updateOne(
                            {item_id:message.item_id},
                            {$set:{
                               'items.hkkjitenzan' :{'data_type' :'number','value' : 0},
                            }
                            }
                        );
                    }
