...
func (t *Question) AddQuestion(c *gin.Context) {
	...
	// 获取系统管理端，可回答问题的用户
	users := userx.GetQuestionUsers(c)
	for uid := range users {

		param := wsx.MessageParam{
			Sender:    req.QuestionerName,
			Recipient: uid,
			Domain:    sessionx.GetSuperDomain(),
			MsgType:   "qa",
		...
		message := "顧客: {" + customerName + "} のユーザー: {" + userName + "} は以下の質問がありました:\n\t <br/>" + "タイトル:" + param.Content + "\n\t<br/>"
...
func (t *Question) ModifyQuestion(c *gin.Context) {
	...
	for uid, uname := range users {
		...
		if sessionx.GetAuthUserID(c) == res.GetQuestion().GetCreatedBy() {
			sender = res.GetQuestion().GetQuestionerName()
			// recipient = res.GetQuestion().GetResponderName()
			recipientID = uid
		} else {
			sender = uname
		...
		if res.GetQuestion().GetStatus() == "open" {
			...
			message := "顧客: {" + customerName + "} のユーザー: {" + userName + "} が以下の問題をクローズしました: " + "\n\t<br/>" + "タイトル: " + res.GetQuestion().GetTitle() + "\n\t<br/>"
			// 发送邮件
			err := mailx.SendQuestionEmail(db, subject, message, userResponse.GetUser().GetNoticeEmail())
			if err != nil {
				httpx.GinHTTPError(c, SentQuestionEmail, err)
