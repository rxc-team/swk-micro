...
func (i *Item) ImportCsvItem(c *gin.Context) {
	...
	if len(val) == 0 {
		jobx.CreateTask(task.AddRequest{
			...
			Steps:        []string{"start", "save-file", "get-file", "read-file", "check-data", "upload", "end"},
			CurrentStep:  "start",
			Database:     db,
			AppId:        appID,
		})
		...
		if err != nil {
			...
			jobx.ModifyTask(task.ModifyRequest{
				JobId:       jobID,
				Message:     i18n.Tr(sessionx.GetCurrentLanguage(c), "job.J_053"),
...
// ImportCheckItems 直接上传csv文件批量盘点数据
// @Router /import/csv/datastores/{d_id}/check/items [post]
func (i *Item) ImportCheckItems(c *gin.Context) {
	loggerx.InfoLog(c, ActionImportCheckItems, loggerx.MsgProcessStarted)
	...
	jobx.CreateTask(task.AddRequest{
		JobId:        jobID,
		JobName:      "csv file import check items",
		Origin:       "apps." + appID + ".datastores." + datastoreID,
		UserId:       userID,
		ShowProgress: true,
		Message:      i18n.Tr(lang, "job.J_014"),
		TaskType:     "ds-csv-import",
		Steps:        []string{"start", "save-file", "read-file", "check-data", "upload", "end"},
		CurrentStep:  "start",
		Database:     db,
		AppId:        appID,
	})
	...
	if err != nil {
		...
		httpx.GinHTTPError(c, ActionImportCheckItems, err)
		return
	}
	...
	if !filex.CheckSupport("csv", files.Header.Get("content-type")) {
		...
		httpx.GinHTTPError(c, ActionImportCheckItems, errors.New("このファイルタイプのアップロードはサポートされていません"))
		return
	}
	// 文件大小检查
	if !filex.CheckSize(domain, "csv", files.Size) {
		...
		httpx.GinHTTPError(c, ActionImportCheckItems, errors.New("ファイルサイズが設定サイズを超えています"))
		return
	}
	...
	// 文件开始上传
	err = inventoryUpload(c, file.Name)
	if err != nil {
		path := filex.WriteAndSaveFile(domain, appID, []string{err.Error()})
	...
	loggerx.InfoLog(c, ActionImportCheckItems, loggerx.MsgProcessEnded)
	c.JSON(200, httpx.Response{
		Status:  0,
		Message: msg.GetMsg("ja-JP", msg.Info, msg.I004, fmt.Sprintf(httpx.Temp, ItemProcessName, ActionImportCheckItems)),
		Data:    nil,
	})
}
